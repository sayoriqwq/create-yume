...
- [ ] "快速开始" 段显式强调先 `pnpm install`（verifyDepsBeforeRun=error 会挡）
...
- [ ] 确认致谢段链接仍有效
- [ ] `bin` / 运行示例的产物文件名与 Infra 3 选型对齐（`.js` 或 `.mjs`）
...
- [ ] §10 限制表：把 Phase 4-A / 4-B / Phase 5 已完成项删除或改为 ✅；剩余项与 Infra / Code Tier 对齐
- [ ] §11 演进路线：把已完成项标注 ✅ 或移除，把未完成项按新优先级重排

### `docs/status.md`

- 当前已加 snapshot banner。本阶段 **不修改 status.md 正文**（它是历史档案）。
- [ ] 仅确认 banner 指向 `plan/lead.md` 仍然有效。

### `docs/agent-foundation-assessment.md`

- [ ] 确认状态头指向 `plan/infra-3-agent-contract.md` 仍然有效
- [ ] 若 Infra 3 的 `AGENTS.md` 已落地，把 §5.1 A 标 ✅
...
- [ ] 若 Infra 3 已落地，确认 `AGENTS.md` 里"支持的项目类型"与 Phase 1 删除 Node 后的状态一致
